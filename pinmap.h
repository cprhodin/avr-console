...
{
         if (pin >= 24) set_bit(&PORTD, pin - 24);
    else if (pin >= 16) set_bit(&PORTC, pin - 16);
    else if (pin >=  8) set_bit(&PORTB, pin -  8);
}

static __inline void pin_set_ddr(pin_t pin)
{
         if (pin >= 24) set_bit(&DDRD, pin - 24);
    else if (pin >= 16) set_bit(&DDRC, pin - 16);
    else if (pin >=  8) set_bit(&DDRB, pin -  8);
}

static __inline void pin_set_pin(pin_t pin)
{
         if (pin >= 24) set_bit(&PIND, pin - 24);
    else if (pin >= 16) set_bit(&PINC, pin - 16);
    else if (pin >=  8) set_bit(&PINB, pin -  8);
}

static __inline void pin_clear(pin_t pin)
{
         if (pin >= 24) clear_bit(&PORTD, pin - 24);
    else if (pin >= 16) clear_bit(&PORTC, pin - 16);
    else if (pin >=  8) clear_bit(&PORTB, pin -  8);
}

static __inline void pin_clear_ddr(pin_t pin)
{
         if (pin >= 24) clear_bit(&DDRD, pin - 24);
    else if (pin >= 16) clear_bit(&DDRC, pin - 16);
    else if (pin >=  8) clear_bit(&DDRB, pin -  8);
}

static __inline void pin_clear_pin(pin_t pin)
{
         if (pin >= 24) clear_bit(&PIND, pin - 24);
    else if (pin >= 16) clear_bit(&PINC, pin - 16);
    else if (pin >=  8) clear_bit(&PINB, pin -  8);
}

static __inline void pinmap_set(pinmap_t pinmap)
{
    if (pinmap & (0xffUL << 24)) set_bits(&PORTD, pinmap >> 24);
    if (pinmap & (0xffUL << 16)) set_bits(&PORTC, pinmap >> 16);
    if (pinmap & (0xffUL <<  8)) set_bits(&PORTB, pinmap >>  8);
}

static __inline void pinmap_set_ddr(pinmap_t pinmap)
{
    if (pinmap & (0xffUL << 24)) set_bits(&DDRD, pinmap >> 24);
    if (pinmap & (0xffUL << 16)) set_bits(&DDRC, pinmap >> 16);
    if (pinmap & (0xffUL <<  8)) set_bits(&DDRB, pinmap >>  8);
}

static __inline void pinmap_set_pin(pinmap_t pinmap)
{
    if (pinmap & (0xffUL << 24)) set_bits(&PIND, pinmap >> 24);
    if (pinmap & (0xffUL << 16)) set_bits(&PINC, pinmap >> 16);
    if (pinmap & (0xffUL <<  8)) set_bits(&PINB, pinmap >>  8);
}

static __inline void pinmap_set_did(pinmap_t pinmap)
{
    if (pinmap & (0x3fUL << 16)) set_bits(&DIDR0, (pinmap >> 16) & 0x3f);
}

static __inline void pinmap_clear(pinmap_t pinmap)
{
    if (pinmap & (0xffUL << 24)) clear_bits(&PORTD, pinmap >> 24);
    if (pinmap & (0xffUL << 16)) clear_bits(&PORTC, pinmap >> 16);
    if (pinmap & (0xffUL <<  8)) clear_bits(&PORTB, pinmap >>  8);
}

static __inline void pinmap_clear_ddr(pinmap_t pinmap)
{
    if (pinmap & (0xffUL << 24)) clear_bits(&DDRD, pinmap >> 24);
    if (pinmap & (0xffUL << 16)) clear_bits(&DDRC, pinmap >> 16);
    if (pinmap & (0xffUL <<  8)) clear_bits(&DDRB, pinmap >>  8);
}

static __inline void pinmap_clear_pin(pinmap_t pinmap)
{
    if (pinmap & (0xffUL << 24)) clear_bits(&PIND, pinmap >> 24);
    if (pinmap & (0xffUL << 16)) clear_bits(&PINC, pinmap >> 16);
    if (pinmap & (0xffUL <<  8)) clear_bits(&PINB, pinmap >>  8);
}

static __inline void pinmap_clear_did(pinmap_t pinmap)
{
    if (pinmap & (0x3fUL << 16)) clear_bits(&DIDR0, (pinmap >> 16) & 0x3f);
}
