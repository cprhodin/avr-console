 ...
 *  IASCII    - Remove non-ASCII from the input stream.
 *  ONLCR     - Translate transmitted NL to CR-NL.
 */
#define ATTR_MASK (ICRNL | ECHO | ICANON | INONBLOCK | IASCII | ONLCR)

/* Rx attributes */
#define is_icrnl()     bitmask_test(&rx_rb.flags,ICRNL)
#define is_echo()      bitmask_test(&rx_rb.flags,ECHO)
#define is_icanon()    bitmask_test(&rx_rb.flags,ICANON)
#define is_inonblock() bitmask_test(&rx_rb.flags,INONBLOCK)
#define is_iascii()    bitmask_test(&rx_rb.flags,IASCII)

/* Tx attributes */
#define is_onlcr() bitmask_test(&tx_rb.flags,ONLCR >> 8)

/*
 * Internal state variables used for I/O translation and processing.
 */
#define set_onlcr_state()    bitmask_set(&tx_rb.flags,RB_SPARE0_MSK)
#define clr_onlcr_state()  bitmask_clear(&tx_rb.flags,RB_SPARE0_MSK)
#define is_onlcr_state()    bitmask_test(&tx_rb.flags,RB_SPARE0_MSK)
#define set_erase_state1()   bitmask_set(&tx_rb.flags,RB_SPARE1_MSK)
#define set_erase_state2()   bitmask_set(&tx_rb.flags,RB_SPARE2_MSK)
#define clr_erase_state()  bitmask_clear(&tx_rb.flags,RB_SPARE1_MSK|RB_SPARE2_MSK)
#define is_erase_state1()   bitmask_test(&tx_rb.flags,RB_SPARE1_MSK)
#define is_erase_state2()   bitmask_test(&tx_rb.flags,RB_SPARE2_MSK)
 ...
 */
#define tx_enable() bitmask_set_clear(&UCSR0B,_BV(UDRIE0)|_BV(TXEN0),_BV(TXCIE0))

/*
 * Enable transmit complete interrupt.
 */
#define tx_complete() bitmask_set_clear(&UCSR0B,_BV(TXCIE0)|_BV(TXEN0),_BV(UDRIE0))

/*
 * Disable transmitter.
 */
#define tx_disable() bitmask_clear(&UCSR0B,_BV(UDRIE0)|_BV(TXCIE0)|_BV(TXEN0))

/*
 * Enable receiver.
 */
#define rx_enable() bitmask_set(&UCSR0B,_BV(RXEN0)|_BV(RXCIE0))

/*
 * Disable receive buffer full interrupt.
 */
#define rx_disable() bitmask_clear(&UCSR0B,_BV(RXCIE0))
...
ISR(USART_RX_vect)
{
    ...
    if (is_icanon()) {
        ...
        default:
            /*
             * Discard non-printable characters not otherwise processed.
             */
            if (is_iascii() && (c < ' ')) return;
            break;
        }
    }
...
void console_init(void)
{
     ...
     * Default attributes.
     */
    console_setattr(ICRNL | ECHO | ICANON | IASCII | ONLCR);
